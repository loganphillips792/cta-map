...
import {
    useRidershipDailyQuery,
    useRidershipMonthlyQuery,
    useRidershipYearlyQuery,
    useRidershipYearsQuery,
    useRouteStatsQuery,
} from "../hooks/ctaQueries";
...
const StatsPage = () => {
    ...
    const filteredStats = useMemo(() => {
        ...
        if (search.trim()) {
            ...
            result = result.filter(
                (r) => r.routeNumber.toLowerCase().includes(term) || r.routeName.toLowerCase().includes(term),
            );
        }
    ...
    const activeRouteNumbers = routeStats.filter((route) => route.totalActive > 0).map((route) => route.routeNumber);

    const totalBusesOnRoad = routeStats.reduce((sum, route) => sum + route.totalActive, 0);

    const ACTIVE_ROUTES_LIMIT = 10;
    const displayedActiveRoutes = showAllActive ? activeRouteNumbers : activeRouteNumbers.slice(0, ACTIVE_ROUTES_LIMIT);
        ...
        return {
            ...
            xAxis: {
                ...
                axisLabel: {
                    ...
                    interval: showingAllData
                        ? Math.floor(data.length / 20)
                        : showingYearOnly
            ...
            dataZoom:
                showingAllData || showingYearOnly
                    ? [
                          {
                              type: "inside",
                              start: 0,
        ...
        <main className="stats-page">
                            ...
                            <Table.Thead>
                                <Table.Tr>
                                    <Table.Th>
                                        ...
                                        >
                                            Route #{" "}
                                            {sortField === "routeNumber" && (sortDirection === "asc" ? "↑" : "↓")}
                                        </UnstyledButton>
                                    </Table.Th>
                                    <Table.Th>
                                        <UnstyledButton
                                        ...
                                        >
                                            Route Name{" "}
                                            {sortField === "routeName" && (sortDirection === "asc" ? "↑" : "↓")}
                                        </UnstyledButton>
                                    </Table.Th>
                                    ...
                                    <Table.Th ta="center">
                                        ...
                                        >
                                            Total Active{" "}
                                            {sortField === "totalActive" && (sortDirection === "asc" ? "↑" : "↓")}
                                        </UnstyledButton>
                                    </Table.Th>
                                </Table.Tr>
                            </Table.Thead>
                        ...
                        {statsQuery.isLoading ? (
                            ...
                            <>
                                ...
                                {hasMoreActiveRoutes && (
                                    ...
                                    >
                                        {showAllActive ? "Show less" : `Show all ${activeRouteNumbers.length} routes`}
                                    </Button>
                                )}
                            </>
                        )}
            ...
            <Paper p="md" radius="md" withBorder mt="lg">
                <Group justify="space-between" mb="md">
                    ...
                    <Select value={selectedYear} onChange={setSelectedYear} data={yearSelectData} w={150} />
                </Group>
                {(selectedYear === "all" ? yearlyQuery.isLoading : monthlyQuery.isLoading) ? (
                    <Skeleton height={350} />
                ) : (
                    <ReactECharts option={ridershipChartOptions} style={{ height: 350 }} opts={{ renderer: "svg" }} />
                )}
            </Paper>
            ...
            <Paper p="md" radius="md" withBorder mt="lg">
                ...
                {dailyQuery.isLoading ? (
                    ...
                    <ReactECharts option={dailyChartOptions} style={{ height: 400 }} opts={{ renderer: "svg" }} />
                )}
            </Paper>
        </main>
    );
